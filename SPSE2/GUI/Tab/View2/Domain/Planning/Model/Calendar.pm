package SPSE2::GUI::Tab::View2::Domain::Planning::Model::Calendar;
...
use Class::MethodMaker
  ...
  [
   ...
   qw / RegisteredWorldNames MyTempAgent MyDomain MyProblem MyLight
	...
	EntryData MyGraph DependencyGraph MyEasySayer GMailUsername
	GMailPassword /
...
sub Sync {
  ...
  my $pseentryidcounter = $args{PSEEntryIDCounter};
  ...
  if ($method eq "Google") {
    ...
    my $res = $self->GetEvents
      (
       Filter => $args{Filter} || {"Andrew Dougherty" => 1},
       Username => $username,
       Password => $password,
      );
    ...
    foreach my $gcalentry (@{$res->{Events}}) {
      # print SeeDumper([$gcalentry->when]);
	...
	my $entryrelation = ["entry-fn","pse",$pseentryid2counter++];
  ...
  return {
	  ...
	  PSEEntryIDCounter => $pseentryidcounter,
	 };
}
...
sub GetCalendar {
  ...
  return Cal::DAV->new
    (
     user => $args{Username},
     pass => $args{Password},
     url => "http://<REDACTED>".$args{Calendar},
    )->cal;
}
...
sub GetEvents {
  ...
  foreach my $cal ($calendar->get_calendars) {
    if (exists $args{Filter}->{$cal->title}) {
...
sub ListGMailCalendars {
  my ($self,%args) = @_;
  my $calendar = Net::Google::Calendar->new;
  $calendar->login($args{Username},$args{Password});
  my @titles;
  foreach my $cal ($calendar->get_calendars) {
    push @titles, $cal->title;
  }
  return {
	  Success => 1,
	  Titles => \@titles,
